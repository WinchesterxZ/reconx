...
func (m *Module) runMantra(ctx context.Context, input string) {
	...
	r := runner.Run(ctx, "mantra", nil,
		...
		runner.WithLineCallback(func(line string) {
			...
			m.store.AddSecret(&store.Secret{Type: t, Value: truncate(line, 200), Source: "mantra"})
			secretCount++
			m.log.Secret(t, "mantra", truncate(line, 80))
		}))
...
func (m *Module) runJsecret(ctx context.Context, input string) {
	...
	r := runner.Run(ctx, "jsecret", nil,
		...
		runner.WithLineCallback(func(line string) {
			...
			t := classifySecret(line)
			m.store.AddSecret(&store.Secret{Type: t, Value: truncate(line, 200), Source: "jsecret"})
			secretCount++
			m.log.Secret(t, "jsecret", truncate(line, 80))
		}))
...
func (m *Module) runTrufflehog(ctx context.Context) {
	...
	r := runner.Run(ctx, tcfg.Path, args,
		...
		runner.WithLineCallback(func(line string) {
			...
			t := jsonStr(line, "DetectorName")
			if t == "" {
				t = jsonStr(line, "detector_name")
			}
			if t == "" {
				t = "unknown"
			}
			raw := jsonStr(line, "Raw")
			if raw == "" {
				raw = truncate(line, 200)
			}
			m.store.AddSecret(&store.Secret{Type: t, Value: truncate(raw, 200), Source: "trufflehog"})
			secretCount++
			m.log.Secret(t, "trufflehog (verified)", truncate(raw, 60))
...
func (m *Module) runTrufflehogGitHub(ctx context.Context) {
	...
	r := runner.Run(ctx, "trufflehog", args,
		...
		runner.WithLineCallback(func(line string) {
			...
			m.store.AddSecret(&store.Secret{Type: t, Value: truncate(line, 200), Source: "trufflehog-github"})
			secretCount++
