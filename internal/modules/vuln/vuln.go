	...
	for _, cat := range categories {
		...
		r := runner.Run(ctx, nucleiPath, args,
			...
			runner.WithStderrCallback(func(line string) {
				...
				m.log.Debug("nuclei[%s]: %s", cat.name, truncate(line, 120))
			}),
...
func parseNucleiLine(line string) *store.Finding {
	...
	name := jsonStr(line, "name")
	if name == "" {
		name = jsonStr(line, "template-id")
	}
	if name == "" {
		name = jsonStr(line, "templateID")
	}
	if name == "" {
		return nil
	}

	severity := strings.ToLower(jsonStr(line, "severity"))
	if severity == "" {
		severity = "info"
	}
	...
	target := jsonStr(line, "matched-at")
	if target == "" {
		target = jsonStr(line, "host")
	}
	if target == "" {
		target = jsonStr(line, "url")
	}
	...
	return &store.Finding{
		...
		Severity: severity,
		Target:   target,
		Template: jsonStr(line, "template-id"),
	}
}
